...
public class ObjectList extends JList {
	...
	private Object addItem(Object o){
		...
		for(int index = 0; index <  size; index++){
			...
			System.out.println("curItem: " + curItem.onlineStatus.toString());
			System.out.println("newItem: " + newItem.onlineStatus.toString());
			if(newItem.uid.equals(curItem.uid)){
				//fmod.deleteElementAt(index);
				fmod.replaceElement(index, newItem);
				//curItem = newItem;
				System.out.println("new curItem: " + ((FacebookUser)fmod.getRealElementAt(index)).onlineStatus.toString());
				return o;
			}
		}
	...
	public void update() {
		...
		while(it.hasNext()){
			...
			System.out.println("status: " + fu.onlineStatus.toString());
			addItem(fu);
		}
		this.repaint();
	}
